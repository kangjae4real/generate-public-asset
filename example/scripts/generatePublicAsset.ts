...
const makeComponent = ({ outputComponentName, withAlias }: Pick<ComponentExport, "outputComponentName" | "withAlias">) => {
...
program
  ...
  .option("-WA, --withAlias", "Output component import path use alias | alias is '@/'", false);
...
const result = fileList.filter((fileName) => fileName.includes(".")).map((fileName) => `/${fileName}`);
